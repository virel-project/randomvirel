...
int main() {
	...
	auto test_a = [&] {
		...
		std::cout << std::endl;
		assert(equalsHex(hash, "a90e9e2a6bb74a0e3a23da303805c36a150a67d293f8fe8bc6f982d7b445568f"));
	};
	...
	auto test_b = [&] {
		...
		outputHex(std::cout, (const char*)hash, sizeof(hash));
		std::cout << std::endl;
		assert(equalsHex(hash, "eb0aad09fcefb60bb991d2459a97c5098b0d9e8def7033400a46884864890064"));
	};
	...
	auto test_c = [&] {
		...
		outputHex(std::cout, (const char*)hash, sizeof(hash));
		std::cout << std::endl;
		assert(equalsHex(hash, "e4e48f5fe3bb58d9f8e07f8d130bb4e441b98e7102751400437551426e3f337c"));
	};
	...
	auto test_d = [&] {
		...
		outputHex(std::cout, (const char*)hash, sizeof(hash));
		std::cout << std::endl;
		assert(equalsHex(hash, "56052c254fc0e0dfbc80cb38fb6cb47872e52cd68048442f9e539e49116202c0"));
	};
	...
	auto test_e = [&] {
		...
		outputHex(std::cout, (const char*)hash, sizeof(hash));
		std::cout << std::endl;
		assert(equalsHex(hash, "6c6e908fbb14c3d60126221cc4e1ced7a9d812c565efdd761f52e98f8dbe15e4"));
	};

	runTest("Hash test 1a (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_a);

	runTest("Hash test 1b (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_b);

	runTest("Hash test 1c (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_c);

	runTest("Hash test 1d (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_d);

	runTest("Hash test 1e (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_e);
	...
	runTest("Hash test 2a (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_a);

	runTest("Hash test 2b (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_b);

	runTest("Hash test 2c (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_c);

	runTest("Hash test 2d (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_d);

	runTest("Hash test 2e (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), test_e);
	...
	runTest("Hash batch test", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomVIREL\x03"), []() {
		...
		assert(equalsHex(hash1, "a90e9e2a6bb74a0e3a23da303805c36a150a67d293f8fe8bc6f982d7b445568f"));
		assert(equalsHex(hash2, "eb0aad09fcefb60bb991d2459a97c5098b0d9e8def7033400a46884864890064"));
		assert(equalsHex(hash3, "e4e48f5fe3bb58d9f8e07f8d130bb4e441b98e7102751400437551426e3f337c"));
	});
